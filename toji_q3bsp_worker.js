...
q3bsp.parse = function(src, tesselationLevel) {
    ...
    lightmaps = q3bsp.readLightmaps(header.lumps[14], src);
	verts = q3bsp.readVerts(header.lumps[10], src);
    meshVerts = q3bsp.readMeshVerts(header.lumps[11], src);
...
q3bsp.compileMap = function(verts, faces, meshVerts, lightmaps, shaders, tesselationLevel) {
	...
	// figure out the indices/vertices/uvs/normals for each shader
	// aka each shader will have its own playcanvas model...
	// probably good enough, but maybe there are way better alternatives?
	
	// first, just give each shader its arrays
	...
	// "sort" faces into shaders, so we can easier build the indices/positions/uvs/normals
    ...
    for (var shader of shaders) {
		...
		for (var face of shader.faces) {
			
			// 1=polygons, 2=patches, 3=meshes, 4=billboards
			// http://www.mralligator.com/q3/#Vertexes
			
			// only polygons for testing atm
			if (face.type != 1)
				continue;
			
			
			for (vertex_i=0; vertex_i<face.vertCount; vertex_i++) {
				vert = verts[face.vertex + vertex_i];
				// scale down and flip y/z
				shader.positions.push( vert.pos[0] * 0.01 );
				shader.positions.push( vert.pos[2] * 0.01 );
				shader.positions.push( vert.pos[1] * -0.01 );
				shader.uvs.push( vert.texCoord[0] );
				shader.uvs.push( vert.texCoord[1] );
				//shader.normals.push( vert.normal[0] );
				//shader.normals.push( vert.normal[1] );
				//shader.normals.push( vert.normal[2] );
				shader.normals.push( vert.normal[0] );
				shader.normals.push( vert.normal[2] );
				shader.normals.push( -vert.normal[1] );
			}
			
			face.vertexCounter = vertexCounter;
			
			//vertexCounter += face.meshVertCount;
			vertexCounter += face.vertCount;
		}
		
		// positions/uvs/normals are saved into arrays, now rip the indices
		for (var face of shader.faces) {
			// face.vertexCounter in indices was just for debugging lol
			// shader.indices.push( face.vertexCounter );
			for(var k = 0; k < face.meshVertCount; ++k) {
				shader.indices.push(face.vertexCounter + meshVerts[face.meshVert + k]);
			}
		}
	}
	...
	//
    //
	//q3_positions = new Array(verts.length * 3);
	//var offset = 0;
	//for(var i = 0; i < verts.length; ++i) {
	//	var vert = verts[i];
	//	q3_positions[offset++] = vert.pos[0] * 0.1;
	//	q3_positions[offset++] = vert.pos[2] * 0.1;
	//	q3_positions[offset++] = vert.pos[1] * 0.1;
	//}
	//q3_normals = new Array(verts.length * 3);
	//var offset = 0;
	//for(var i = 0; i < verts.length; ++i) {
	//	var vert = verts[i];
	//	q3_normals[offset++] = vert.normal[0];
	//	q3_normals[offset++] = vert.normal[1];
	//	q3_normals[offset++] = vert.normal[2];
	//}
	//q3_uvs = new Array(verts.length * 3);
	//var offset = 0;
	//for(var i = 0; i < verts.length; ++i) {
	//	var vert = verts[i];
	//	q3_uvs[offset++] = vert.texCoord[0];
	//	q3_uvs[offset++] = vert.texCoord[1];
	//}
	//
    //// Compile index list
    //indices = new Array();
    ...
    //                //shader.indices.push(addMe + meshVerts[face.meshVert + k]);
    //            }
    //            shader.elementCount += face.meshVertCount;
	//				addMe += face.meshVertCount
    //        }
    //    }
